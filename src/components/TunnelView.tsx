import { Polygon, Tunnel, screenHeight, screenWidth } from '@/components/Tunnel';
import '@/styles/tunnel.css';
import { useEffect, useRef, useState } from 'react';
import { Pane } from 'tweakpane';
import '@/styles/TunnelView.css'
...
export default function TunnelView({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const outside = 1000;
    const fillEntireThingPath = `M -${outside} -${outside} L ${
      screenWidth + outside
    } -${outside} L ${screenWidth + outside} ${
      screenHeight + outside
    } L -${outside} ${screenHeight + outside} Z`;
  ...
  return (
    <div className={`am-tunnel-view ${className}`}>
      <svg
        ref={svgRef}
        width={screenWidth}
        height={screenHeight}
